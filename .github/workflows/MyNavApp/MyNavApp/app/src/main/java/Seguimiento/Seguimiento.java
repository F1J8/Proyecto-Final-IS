...
import adapters.ProspAdaptapter;
...
public class Seguimiento extends Fragment {
    ...
    private ProspAdaptapter mAdapter;
    Button Refresh;

    View vista;

    TextView name, tel, dni;
    public Seguimiento() {
    ...
    /** La funcion de consulta va aqui*/

    private void consulta() {
        SQLiteOpenHelper db = new SQLiteOpenHelper(getContext(),"db",null,2);
        mDatabase = db.getWritableDatabase();

        RecyclerView recyclerView = vista.findViewById(R.id.recyclerID);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        mAdapter = new ProspAdaptapter(getContext(), getAllItems());
        recyclerView.setAdapter(mAdapter);
        mAdapter.sCursor(getAllItems());
